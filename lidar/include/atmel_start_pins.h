 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void LED_D13_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTE_set_pin_pull_mode(2, pull_mode);
}

/**
 * \brief Set LED_D13 data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void LED_D13_set_dir(const enum port_dir dir)
{
	PORTE_set_pin_dir(2, dir);
}

/**
 * \brief Set LED_D13 input/sense configuration
 *
 * Enable/disable LED_D13 digital input buffer and pin change interrupt,
 * select pin interrupt edge/level sensing mode
 *
 * \param[in] isc PORT_ISC_INTDISABLE_gc    = Iterrupt disabled but input buffer enabled
 *                PORT_ISC_BOTHEDGES_gc     = Sense Both Edges
 ...
 *                PORT_ISC_LEVEL_gc         = Sense low Level
 */
static inline void LED_D13_set_isc(const PORT_ISC_t isc)
{
	PORTE_pin_set_isc(2, isc);
}

/**
 * \brief Set LED_D13 inverted mode
 *
 * Enable or disable inverted I/O on a pin
 *
 * \param[in] inverted true  = I/O on LED_D13 is inverted
 *                     false = I/O on LED_D13 is not inverted
 */
static inline void LED_D13_set_inverted(const bool inverted)
{
	PORTE_pin_set_inverted(2, inverted);
}

/**
 * \brief Set LED_D13 level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void LED_D13_set_level(const bool level)
{
	PORTE_set_pin_level(2, level);
}

/**
 * \brief Toggle output level on LED_D13
 *
 * Toggle the pin level
 */
static inline void LED_D13_toggle_level()
{
	PORTE_toggle_pin_level(2);
}

/**
 * \brief Get level on LED_D13
 *
 * Reads the level on a pin
 */
static inline bool LED_D13_get_level()
{
